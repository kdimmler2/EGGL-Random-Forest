...
aucs <- read.table("results/current_iteration/training/testing_auc.list")
aucs <- aucs$V1
...
print(median_subset)
#median_indices <- which.min(abs(aucs - median_value))
...
vi_path <- paste("results/current_iteration/training/subsets/subset", final_subset, "/Variable_Importance.txt", sep="")
ts_path <- paste("results/current_iteration/training/subsets/subset", final_subset, "/TimesSelected.txt", sep="")
...
outfile <- write.csv(vi_subset, "results/current_iteration/training/feature_importance_scores.txt", row.names=FALSE)
